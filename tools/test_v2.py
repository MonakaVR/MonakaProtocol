...
def cases():
    ...
    for name in ['observation', 'device-state', 'mtp-pose', 'tracker-state']:
        ...
        if p['type'] == 'pose':
            for field in ['linear_velocity', 'angular_velocity', 'linear_acceleration']:
        ...
        if p['type'] == 'pose':
            for modality in ['rotation_only', 'none']:
                ...
                if 'orientation_evidence' in q and modality == 'none':
                    q['orientation_evidence'] = 'none'
        ...
        if 'input' in p:
            q = copy.deepcopy(p)
            q['input']['source_id'] = 'wrong-source'
...
def resolve_cpp_runner(explicit):
    if explicit:
        path = Path(explicit)
        if not path.is_absolute():
            path = R / path
        require(path.is_file(), f'v2 C++ runner not found: {path}')
        return path
    candidates = [
        R / 'build/cpp/Release/monaka_codec_runner_v2.exe',
        R / 'build/cpp/monaka_codec_runner_v2',
        R / 'build/cpp/Release/monaka_codec_runner_v2',
        R / 'build/cpp/monaka_codec_runner_v2.exe',
    ]
    for path in candidates:
        if path.is_file():
            return path
    raise RuntimeError('v2 C++ runner not found; build CMake tests first or pass --cpp PATH')

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--cpp', default=None, help='Path to monaka_codec_runner_v2; auto-detected when omitted')
    parser.add_argument('--generate-only', action='store_true')
    args = parser.parse_args()
    folder = R / 'fixtures/v2'
    ...
    cpp = resolve_cpp_runner(args.cpp)
    cp = os.pathsep.join(str(R / p) for p in [
        'jvm/build/classes/kotlin/test',
        'jvm/build/libs/monaka-protocol-jvm-0.1.0.jar',
        'jvm/libs/gson-2.11.0.jar',
        'jvm/libs/kotlin-stdlib-2.3.10.jar',
    ])
    commands = [[str(cpp.resolve())], ['java', '-cp', cp, 'dev.monaka.protocol.v2.Runner']]
    scratch = R / 'build/cross-v2'
    ...
    for item in index:
        ...
        for i, command in enumerate(commands):
            ...
            if item['error']:
                ...
                cross = scratch / f'{i}.json'
                cross.write_text(output, encoding='utf-8')
                require(json.loads(subprocess.check_output(commands[1-i] + [str(cross)], text=True, encoding='utf-8')) == item['decoded'], 'cross semantics')
                directions += 1

    files = [
        p for source_folder in ['cpp/src', 'cpp/include', 'jvm/src', 'schema', 'fixtures/v2']
        for p in (R / source_folder).rglob('*') if p.is_file()
    ]
    ...
    report = {
        ...
        'binary_sha256': {
            cpp.relative_to(R).as_posix(): hashlib.sha256(cpp.read_bytes()).hexdigest(),
            jar.relative_to(R).as_posix(): hashlib.sha256(jar.read_bytes()).hexdigest(),
        },
    }
